    ...
    <div className={`${compact ? 'my-1.5' : 'my-3'} rounded-lg overflow-hidden border border-line bg-[#1a1f27]`}>
      ...
      {html ? (
        ...
        <pre className={`${compact ? 'p-2' : 'p-4'} overflow-x-auto text-[13px] leading-relaxed m-0`}>
          <code className="text-fg-muted font-mono">{code}</code>
        </pre>
      )}
    </div>
  )
...
function MarkdownRendererInner({ children }: { children: string }): React.JSX.Element {
  ...
  const components = useMemo(() => ({
    ...
    code({ className, children, ...props }: { className?: string; children: React.ReactNode }) {
      ...
      if (!isBlock) {
        ...
        if (isFilePath(text)) {
          return (
            <code
              className="rounded bg-overlay-3 px-1.5 py-0.5 text-[0.85em] font-mono text-blue-400/70 hover:text-blue-400 cursor-pointer transition-colors"
        ...
        return (
          <code className="rounded bg-overlay-3 px-1.5 py-0.5 text-[0.85em] font-mono text-fg-strong" {...props}>
            {children}
          </code>
        )
      }
    ...
    h1({ children }: { children: React.ReactNode }) {
      return <h1 className={`text-xl font-bold text-fg-strong first:mt-0 ${compact ? 'mt-3 mb-1' : 'mt-5 mb-2'}`}>{children}</h1>
    },
    h2({ children }: { children: React.ReactNode }) {
      return <h2 className={`text-base font-semibold text-fg-strong first:mt-0 ${compact ? 'mt-2 mb-1' : 'mt-4 mb-2'}`}>{children}</h2>
    },
    h3({ children }: { children: React.ReactNode }) {
      return <h3 className={`text-sm font-semibold text-fg-strong first:mt-0 ${compact ? 'mt-1.5 mb-0.5' : 'mt-3 mb-1'}`}>{children}</h3>
    },
    ...
    em({ children }: { children: React.ReactNode }) {
      return <em className="italic text-fg-muted">{children}</em>
    },
    ...
    li({ children }: { children: React.ReactNode }) {
      return <li className="text-fg-strong leading-relaxed">{children}</li>
    },
    blockquote({ children }: { children: React.ReactNode }) {
      return <blockquote className={`border-l-2 border-line-strong pl-3 italic text-fg-subtle ${compact ? 'my-1.5' : 'my-3'}`}>{children}</blockquote>
    },
    ...
    hr() { return <hr className="border-line-strong my-4" /> },
    ...
    th({ children }: { children: React.ReactNode }) {
      return <th className="text-left py-1.5 px-3 text-fg-muted font-medium text-xs uppercase tracking-wide">{children}</th>
    },
    td({ children }: { children: React.ReactNode }) {
      return <td className="py-1.5 px-3 border-b border-line-soft text-fg-muted">{children}</td>
    }
  }), [compact])
