    ...
    >
      <div className="w-full max-w-2xl max-h-[70vh] flex flex-col rounded-2xl bg-surface-3 border border-line-strong shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-line-soft">
          <h2 className="text-sm font-semibold text-fg-strong">Code Blocks</h2>
          <button
            onClick={onClose}
        ...
        {/* Body */}
        {blocks.length === 0 ? (
          <div className="p-8 text-center text-fg-subtle text-sm">No code blocks found in this session.</div>
        ) : (
          <div className="overflow-y-auto flex-1 divide-y divide-line-soft">
            {blocks.map((block) => (
              <div key={block.index} className="flex items-start gap-3 px-5 py-3 hover:bg-overlay-1 transition-colors">
                <span className="mt-0.5 flex-shrink-0 rounded px-1.5 py-0.5 text-[10px] font-mono font-medium uppercase tracking-wide bg-overlay-3 text-fg-subtle min-w-[42px] text-center">
                  {block.lang}
                </span>
                <pre className="flex-1 font-mono text-[11px] text-fg-muted whitespace-pre-wrap leading-relaxed overflow-hidden" style={{ maxHeight: '2.6rem' }}>
                  {block.preview}
                </pre>
                <button
                  ...
                  className={`flex-shrink-0 rounded px-2.5 py-1 text-[11px] font-medium border transition-colors mt-0.5 ${
                    copiedIndex === block.index
                      ...
                      : 'border-line text-fg-subtle hover:text-fg-strong hover:border-line-strong'
                  }`}
                >
